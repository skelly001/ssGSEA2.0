...
\nError message returned by 'cmapR::parse.gctx()':\n\n", dataset, '\n\n')
  ...
  if(length(keep.idx) == 0) {  
    ...
    warning('No overlap to any gene sets found in your data set!\nPossible reasons: 1) organism other than human; 2) wrong format of gene names/site ids!\n')
    return(NULL)
  } else {
    ## #####################################################
    ## update all data
    ...
    if (length(locs) == 0) { 
      ...
      #################################################
      ## score matrix
      if(nrow(score.matrix.2) == 1) { #  There is a parse error when reading the GCT files with only 1 row, so add create a second row if only 1 gene set is found 
        sm2 <- rbind(data.matrix(score.matrix.2), data.matrix(score.matrix.2))
        rownames(sm2) <- c(rownames(sm2)[1], "blank")
        
        gs.names.2 <- c(gs.names.2, "blank")
        names(gs.names.2) <- c(names(gs.names.2)[1], "blank")
        
        gs.descs.2 <- rbind(gs.descs.2, gs.descs.2)
        rownames(gs.descs.2) <- c(rownames(gs.descs.2)[1], "blank")
      } else {
        sm2 <- data.matrix(score.matrix.2)
      }
      V.GCT <- new('GCT')
      V.GCT@mat <- sm2
      ...
      #################################################
      ## p-value matrix
      if(nrow(pval.matrix.2) == 1) {
        pm2 <- rbind(data.matrix(pval.matrix.2), data.matrix(pval.matrix.2))
        rownames(pm2) <- c(rownames(pm2)[1], "blank")
      } else {
        pm2 <- data.matrix(pval.matrix.2)
      }
      P.GCT <- new('GCT')
      P.GCT@mat <- pm2
      ...
      ## ##############################################  
      ## FDR matrix
      if(nrow(fdr.matrix.2) == 1) {
        fm2 <- rbind(data.matrix(fdr.matrix.2), data.matrix(fdr.matrix.2))
        rownames(fm2) <- c(rownames(fm2)[1], "blank")
      } else {
        fm2 <- data.matrix(fdr.matrix.2)
      }
      F.GCT <- new('GCT')
      F.GCT@mat <- fm2 #F.GCT.mat
