...
for(i in names(gct.files)){
  ...
  if (!is.null(gsea.res)) {
    ...
    msg <- as.data.frame("No overlap to any gene sets found in your data set.")
    write_csv(msg, file = "No overlap to any gene sets found in your data set.csv")
    setwd(paste(out.dir, date.str, sep = "\\"))
  }
}
